...
class TecDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        _input = torch.cat(tuple(input_ori[:self.window_size]), 1)
        # 
        day = sum([self.mon_day[i] for i in range(1,self.tar_date[idx][1])]) + self.tar_date[idx][2]
        tar_date = torch.tensor([[math.sin(self.tar_date[idx][1]/12), math.cos(self.tar_date[idx][1]/12), math.sin((day)/366), math.cos(day/366), \
        math.sin(self.tar_date[idx][3]/24), math.cos(self.tar_date[idx][3]/24)]for i in range(71)], dtype=torch.float)
...
if __name__ == '__main__':
    ...
    '''
    tmpdata = TecDataset('txt/valid_2020/', data_type='dir', pred_future=True)
    ...
    print(tmpdataloader)
    for inp_map, tar_map, date, _ in tmpdataloader:
        #print(inp_map, tar_map[:,0,:,:].size())
        print(inp_map.size(), tar_map.size())
        #input()